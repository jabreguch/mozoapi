...
public interface IHttpClientSoporte : IHttpClientGeneric
{
    //Task<SelectList> Inventario_Almacen_GetAll(ArticuloStockModel c, string CoAlmacen=null);
}

public class HttpClientSoporte : IHttpClientSoporte
{
    private readonly IMemoryCache _cache;
    private readonly HttpClient _client;
    private readonly ConfigurationModel _configuration;

    public HttpClientSoporte(HttpClient client, IMemoryCache cache, IOptions<ConfigurationModel> configuration)
    {
        _client = client;
        _cache = cache;
    ...
    //{
    //    IEnumerable<ArticuloStockModel> list = await HttpClientUtil.All<IEnumerable<ArticuloStockModel>>(_client, "Inventario/AlmacenApi/GetAllActive", c);
    //    return new SelectList(list, "CoAlmacen", "NoAlmacen", CoAlmacen, "NoLocal");
    //}
}